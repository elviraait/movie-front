...
export default function MoviePage() {
  ...
  const [imgError, setImgError] = useState(false); // 👈 добавили

  const user = getUserInfo();
  const admin = isAdmin();

  useEffect(() => {
    apiGetMovieWithReviews(id)
      .then(data => {
        setMovie(data);
        setImgError(false); // сбрасываем при смене фильма
      })
      .catch(() => {})
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {getAccessToken() && (
        <div className="card fade-up" style={{ marginBottom: 32 }}>
          <h3 style={{ fontFamily: 'Bebas Neue, cursive', fontSize: 22, marginBottom: 16 }}>
            Write a Review
          </h3>
